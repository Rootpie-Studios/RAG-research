...
CONFIG_FILE_NAME = "config_mistral.py" # <-- NEW: Define config file name here

def read_config_variables():
    # Construct the full path to config_mistral.py
    config_path = os.path.join(current_script_dir, CONFIG_FILE_NAME) # <-- UPDATED
    ...
    try:
        ...
        # Corrected error message to reflect the new config file name
        print(f"{COLOR_RED}Error: {CONFIG_FILE_NAME} not found.{COLOR_RESET}") # <-- UPDATED
        exit()
    return config_vars
...
def update_config_file(variable_name, old_value, new_value):
    ...
    try:
        ...
        # Corrected success message to reflect the new config file name
        print(
            f"{COLOR_BLUE}Updated {variable_name} in {CONFIG_FILE_NAME} from {old_value} to {new_value}.{COLOR_RESET}" # <-- UPDATED
        )
    except Exception as e:
        # Corrected error message to reflect the new config file name
        print(f"{COLOR_RED}Error updating {CONFIG_FILE_NAME}: {e}{COLOR_RESET}") # <-- UPDATED
...
def main():
    ...
    scripts = {
        "1": "parse_mistral_embedd_into_db.py", # <-- UPDATED to new script name
