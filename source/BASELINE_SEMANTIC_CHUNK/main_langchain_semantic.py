...
# Instantiate OpenAIEmbeddings from LangChain
embeddings = OpenAIEmbeddings(api_key=OPENAI_KEY, model=EMBEDDING_MODEL_NAME)
...
def chunk_pdf_semantically(pdf_path, embeddings_model):
    """
    Parses a PDF and chunks it semantically using LangChain's SemanticChunker.
    """
    filename = os.path.basename(pdf_path)
    text_and_pagenumber = parse_document(pdf_path)
    
    # Using SemanticChunker from LangChain
    text_splitter = SemanticChunker(embeddings_model)
    
    chunks = []
    chunk_index = 1
    # Process each page separately to maintain page number information
    for page_number, page_text in text_and_pagenumber:
        ...
        for page_chunk in page_chunks:
            ...
            chunks.append({"text": page_chunk.page_content, "metadata": chunk_metadata})
            chunk_index += 1
...
def process_pdfs_and_insert(directory, client, collection, embeddings_model, max_workers=None):
    if max_workers is None:
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".pdf"):
            ...
            chunks = chunk_pdf_semantically(pdf_path, embeddings_model)
