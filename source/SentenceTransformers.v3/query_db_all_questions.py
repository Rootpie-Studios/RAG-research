...
def get_text_match_info(value, document):
    ...
    answer_text = normalize_text(value["answer"].lower())
    # print(f"Answer text: {answer_text}")
    answer_list = list(answer_text)
...
def query_documents_all_embeddings(question_dict, n_results):
    all_rows = []
    for value in question_dict.values():
        ...
        for idx, document in enumerate(results["documents"][0]):
            ...
            guessed_file = metadata.get(
                "filename"
            ).lower()  # since toml parser might change case
            filename_match = guessed_file == correct_file

            correct_pages = value["files"][0]["page_numbers"]
            guessed_page = metadata.get("page_number")

            # Convert guessed_page to a string before splitting, if it's not already a string
            if isinstance(guessed_page, int):
                guessed_page_str = str(guessed_page)
            else:
                guessed_page_str = guessed_page  # Assume it's already a string or handle other types as needed

            guessed_page_list = list(map(int, guessed_page_str.split(",")))
...
def query_documents_all_embeddings_parallel(question_dict, n_results):
...
if __name__ == "__main__":  # Needed for multiprocessing to work correctly
    ...
    # --------------------------------------------------------------#
    question_dict = get_embedded_questions(TOML_DIRECTORY_CLEANED)
